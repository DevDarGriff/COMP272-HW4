 ...
 class myHashMap<K,V> {
     ...
     public V remove(K key) {
          ...
          */
          ...
          while (current != null) {
            if (current.key.equals(key)) {                  //if HashMap contains key
                if (prev == null) {                         //prev is null
                    bucket.set(index, current.next);        //set bucket index to current's next
                } else {
                    prev.next = current.next;               //set prev's next to current.next's value
                }
                size--;                                     //decrease size of after node removal
                return current.value;                       //return current's value
            }
            prev = current;                                 //set prev to current
            current = current.next;                         //set current to current's next
          }

         return null;                                       //key not in HashMap, return null
     }
     ...
     public V replace(K key, V val) {
          ...
          */
          ...
          while (current != null) {
            if (current.key.equals(key)) {              //if current key == key
                V oldVal = current.value;               //set oldVal to current's val
                current.value = val;                    //set current' val to val passed in 
     ...
     public boolean replace(K key, V oldVal, V newVal) {
          ...
          */
          ...
          while (current != null) {
            //if current's key = key passed in AND current val = oldVal passed in
            if (current.key.equals(key) && current.value.equals(oldVal)) {
                replace(key, newVal);                       //replace key w/ newVal
                return true;                                
            }
            current = current.next;                         //set current to current's next
          }
